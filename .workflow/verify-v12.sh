#!/bin/bash
echo "=== Apply v12 (rewrite strip /api) ==="
sudo -n bash -c "cp /tmp/sim-kk-v12.conf /etc/nginx/sites-available/sim-kk && nginx -t 2>&1 && systemctl reload nginx"
sleep 1
echo ""
...
echo "GET /api/dbg.php:"
curl -s http://43.133.142.74/api/dbg.php
echo ""
echo ""
echo "=== Test login ==="
