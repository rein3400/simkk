#!/bin/bash
echo "=== Apply v18 (named @laravel + absolute SCRIPT_FILENAME) ==="
sudo -n bash -c "cp /tmp/sim-kk-v18.conf /etc/nginx/sites-available/sim-kk && nginx -t 2>&1 && systemctl reload nginx"
sleep 1
echo "=== /api/login (POST) ==="
